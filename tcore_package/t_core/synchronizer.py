...
from messages import TransformationException, NIL_PACKET, Packet
...
class Synchronizer(ControlPrimitive):
    ...
    def _custom_merge(self):
        ...
        '''
        merged_packet = self.custom_merge(self.success)
        if not isinstance(merged_packet, Packet):
            raise TransformationException('You must return a single packet when merging')
